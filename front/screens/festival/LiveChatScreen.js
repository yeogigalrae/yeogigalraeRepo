...
export default LiveChatScreen = ({route}) => {
  ...
  return (
    <View style={LiveChatStyle.liveChat}>
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "position" : null} keyboardVerticalOffset={Platform.OS === "ios" ? 90 : 0}>
        <ScrollView style={LiveChatStyle.liveChatArea} onScroll={handleScroll} scrollEventThrottle={16}>
          {messageList?.map((data, idx) => {
            return <Message data={data} key={idx} />;
          })}
        </ScrollView>
        <View style={LiveChatStyle.liveChatInputBox}>
          <TextInput
          ...
          >
            <Image style={LiveChatStyle.liveChatInputButtonImage}></Image>
          </TouchableOpacity>
        </View>
